...
const decodeOrchestrationStreamingMessageInput = Schema.decodeUnknownEffect(
  OrchestrationStreamingMessageInput,
);
...
it.effect("accepts orchestration streaming messages for steer delivery", () =>
  Effect.gen(function* () {
    const parsed = yield* decodeOrchestrationStreamingMessageInput({
      threadId: "thread-1",
      input: "redirect to inspect reconnect logs first",
      attachments: [],
      streamingBehavior: "steer",
    });
    assert.strictEqual(parsed.threadId, "thread-1");
    assert.strictEqual(parsed.streamingBehavior, "steer");
  }),
);
