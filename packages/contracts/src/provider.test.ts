...
describe("ProviderSendTurnInput", () => {
  ...
  it("accepts streamingBehavior for steer and follow-up delivery", () => {
    const steer = decodeProviderSendTurnInput({
      threadId: "thread-1",
      input: "redirect now",
      streamingBehavior: "steer",
    });
    const followUp = decodeProviderSendTurnInput({
      threadId: "thread-1",
      input: "after this, also fix tests",
      streamingBehavior: "followUp",
    });

    expect(steer.streamingBehavior).toBe("steer");
    expect(followUp.streamingBehavior).toBe("followUp");
  });
});
