...
- Do it so well that the user is genuinely impressed, not politely satisfied.
...
- Search before building. Reuse shared code where possible.
...
- NEVER use destructive commands like `git reset --hard`, `git checkout .`, `git clean -fd`, or `git stash` unless the user explicitly confirms they want to override this rule.
- If rebase or merge conflicts touch files you did not modify, stop and ask the user.
