    ...
    const appLayer = HttpRouter.serve(makeRoutesLayer, {
      ...
      Layer.provide(
        Layer.mock(ProviderService)({
          ...
          sendTurn: () =>
            Effect.succeed({ threadId: defaultThreadId, turnId: TurnId.makeUnsafe("turn-1") }),
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("serves orchestration snapshots over HTTP", () =>
    Effect.gen(function* () {
      ...
      assert.equal(response.status, 200);
      assert.deepStrictEqual(JSON.parse(yield* response.text), snapshot);
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );

  it.effect("dispatches orchestration commands over HTTP", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const workspaceRoot = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "t3-http-dispatch-workspace-",
      });
      const receivedCommands: Array<OrchestrationCommand> = [];
      const command: Extract<OrchestrationCommand, { type: "project.create" }> = {
        type: "project.create",
        commandId: CommandId.makeUnsafe("command-http-1"),
        projectId: ProjectId.makeUnsafe("project-http-1"),
        title: "HTTP Project",
        workspaceRoot,
        defaultModelSelection: {
          provider: "codex",
        ...
        layers: {
          orchestrationEngine: {
            dispatch: (nextCommand) =>
              Effect.sync(() => {
                receivedCommands.push(nextCommand);
                return { sequence: 41 };
              }),
          },
        },
      ...
      const response = yield* HttpClient.post("/api/orchestration/dispatch", {
        ...
        body: HttpBody.text(JSON.stringify(command), "application/json"),
      });

      assert.equal(response.status, 200);
      assert.deepStrictEqual(JSON.parse(yield* response.text), { sequence: 41 });
      assert.deepStrictEqual(receivedCommands, [command]);
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
