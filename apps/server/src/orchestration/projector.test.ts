...
describe("orchestration projector", () => {
  ...
  it("keeps completed assistant messages terminal when a late streaming delta arrives", async () => {
    const createdAt = "2026-02-23T09:30:00.000Z";
    const completeAt = "2026-02-23T09:30:02.000Z";
    const lateDeltaAt = "2026-02-23T09:30:03.000Z";
    const model = createEmptyReadModel(createdAt);
    ...
    const afterComplete = await Effect.runPromise(
      projectEvent(
        ...
        makeEvent({
          ...
          aggregateId: "thread-1",
          occurredAt: completeAt,
          commandId: "cmd-complete-terminal",
          payload: {
            threadId: "thread-1",
            messageId: "assistant:msg-terminal",
            role: "assistant",
            text: "done",
            turnId: "turn-1",
            streaming: false,
    ...
    const afterLateDelta = await Effect.runPromise(
      projectEvent(
        ...
        makeEvent({
          ...
          aggregateKind: "thread",
          aggregateId: "thread-1",
          occurredAt: lateDeltaAt,
          commandId: "cmd-late-delta-terminal",
          payload: {
            threadId: "thread-1",
            messageId: "assistant:msg-terminal",
            role: "assistant",
            ...
            turnId: "turn-1",
            streaming: true,
            createdAt: lateDeltaAt,
            updatedAt: lateDeltaAt,
          },
        }),
      ),
    );

    const message = afterLateDelta.threads[0]?.messages[0];
