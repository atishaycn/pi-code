...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("ignores assistant item completion with no visible text", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    expect(thread?.messages.some((message) => message.id === "assistant:item-empty")).toBe(false);
  });
  ...
  it("keeps session running until turn.completed finalizes buffered assistant output", async () => {
    const harness = await createHarness({ serverSettings: { enableAssistantStreaming: false } });
    const now = new Date().toISOString();

    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-turn-started-finalize-before-ready"),
      provider: "codex",
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-finalize-before-ready"),
    });
    ...
    await waitForThread(
      ...
      (thread) =>
        thread.session?.status === "running" &&
        thread.session?.activeTurnId === "turn-finalize-before-ready",
    );

    harness.emit({
      type: "content.delta",
      eventId: asEventId("evt-message-delta-finalize-before-ready"),
      provider: "codex",
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-finalize-before-ready"),
      itemId: asItemId("item-finalize-before-ready"),
      payload: {
        streamKind: "assistant_text",
        delta: "late final text",
      },
    });
    harness.emit({
      type: "turn.completed",
      eventId: asEventId("evt-turn-completed-finalize-before-ready"),
      provider: "codex",
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-finalize-before-ready"),
    ...
    await waitForThread(
      ...
      (entry) =>
        ...
        entry.messages.some(
          (message: ProviderRuntimeTestMessage) =>
            message.id === "assistant:item-finalize-before-ready" &&
            !message.streaming &&
            message.text === "late final text",
        ),
    );
    ...
    const readyEventIndex = events.findLastIndex(
    ...
    const assistantCompleteIndex = events.findIndex(
      (event) =>
        ...
        event.payload.messageId === "assistant:item-finalize-before-ready" &&
        event.payload.streaming === false,
    );
    expect(assistantCompleteIndex).toBeGreaterThanOrEqual(0);
    expect(readyEventIndex).toBeGreaterThan(assistantCompleteIndex);
  });
  ...
  it("projects Codex task lifecycle chunks into thread activities", async () => {
    ...
    expect(progress?.summary).toBe("Code reviewer is validating the desktop rollout chunks.");
