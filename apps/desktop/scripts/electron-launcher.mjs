// This file mostly exists because we want dev mode to say "Pi Code (Dev)" instead of "electron"
...
const APP_BUNDLE_ID = "dev.pi.picode";
...
function ensureElectronInstalled(require) {
  ...
  if (result.status !== 0) {
    throw new Error(
      `Electron install bootstrap failed with exit code ${result.status ?? "unknown"}`,
    );
  }

  if (!existsSync(pathFile) || !existsSync(distDir)) {
    throw new Error(`Electron install bootstrap did not produce expected files in ${electronDir}`);
  }
}
