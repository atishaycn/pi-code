...
import type {
  DesktopOpenTerminalWindowInput,
  DesktopTheme,
...
import { buildDevelopmentRestartCommand, DEFAULT_DESKTOP_RESTART_ROOT } from "./restartApp";
...
const OPEN_TERMINAL_WINDOW_CHANNEL = "desktop:open-terminal-window";
...
const APP_DISPLAY_NAME = isDevelopment ? "Pi Code (Dev)" : "Pi Code (Alpha)";
const APP_USER_MODEL_ID = "dev.pi.picode";
const LINUX_DESKTOP_ENTRY_NAME = isDevelopment ? "picode-dev.desktop" : "picode.desktop";
const LINUX_WM_CLASS = isDevelopment ? "picode-dev" : "picode";
const USER_DATA_DIR_NAME = isDevelopment ? "picode-dev" : "picode";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "Pi Code (Dev)" : "Pi Code (Alpha)";
...
function backendChildEnv(): NodeJS.ProcessEnv {
  return sanitizeDesktopBackendEnv(process.env);
}
...
function getSafeOpenTerminalWindowInput(rawInput: unknown): DesktopOpenTerminalWindowInput | null {
  ...
  const cwd = Path.resolve(candidate.cwd.trim());
...
function buildTerminalLaunchCommand(input: DesktopOpenTerminalWindowInput): string {
  const changeDirectory = `cd ${shellQuote(input.cwd)}`;
  if (!input.command) {
    return changeDirectory;
  }

  return `${changeDirectory}; ${input.command}`;
}

async function openTerminalWindow(input: DesktopOpenTerminalWindowInput): Promise<boolean> {
  if (!FS.existsSync(input.cwd)) {
    return false;
  }

  const command = buildTerminalLaunchCommand(input);
  ...
  try {
    if (process.platform === "darwin") {
      ...
      ChildProcess.execFileSync("/usr/bin/osascript", ["-e", appleScript], {
        stdio: "ignore",
      });
      return true;
    }
    ...
    if (process.platform === "linux") {
      const terminalCommand = `bash -lc ${shellQuote(command + "; exec bash")}`;
      const candidates: ReadonlyArray<readonly [string, string[]]> = [
        ["x-terminal-emulator", ["-e", terminalCommand]],
    ...
    if (process.platform === "win32") {
      const child = ChildProcess.spawn(
        "cmd.exe",
        ["/c", "start", input.title ?? "Pi Code", "cmd.exe", "/k", command],
        {
          cwd: input.cwd,
          detached: true,
          stdio: "ignore",
    ...
    writeDesktopLogHeader(
      `Failed to open terminal window: cwd=${sanitizeLogValue(input.cwd)} error=${sanitizeLogValue(formatErrorMessage(error))}`,
    );
    return false;
  }
...
function resolveBackendCwd(): string {
  return resolveDesktopBackendCwd();
}

function resolveDesktopStaticDir(): string | null {
  const candidates = resolveDesktopStaticDirCandidates({
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Pi Code failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Pi Code ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
...
function registerIpcHandlers(): void {
  ...
  ipcMain.removeHandler(OPEN_TERMINAL_WINDOW_CHANNEL);
  ipcMain.handle(OPEN_TERMINAL_WINDOW_CHANNEL, async (_event, rawInput: unknown) => {
    const input = getSafeOpenTerminalWindowInput(rawInput);
    if (!input) {
      return false;
    }

    return openTerminalWindow(input);
  });

  ipcMain.removeHandler(RESTART_APP_CHANNEL);
  ipcMain.handle(RESTART_APP_CHANNEL, async () => {
    if (app.isPackaged) {
      throw new Error("App restart is only available in development builds.");
    }

    if (!FS.existsSync(DEFAULT_DESKTOP_RESTART_ROOT)) {
      throw new Error(`Restart root does not exist: ${DEFAULT_DESKTOP_RESTART_ROOT}`);
    }

    const restartCommand = buildDevelopmentRestartCommand(DEFAULT_DESKTOP_RESTART_ROOT);
    const restartProcess = ChildProcess.spawn("/bin/bash", ["-lc", restartCommand], {
      cwd: DEFAULT_DESKTOP_RESTART_ROOT,
      detached: true,
      stdio: "ignore",
      env: { ...process.env },
    });
...
function resolveDesktopRootUrl(): string {
...
function createWindow(): BrowserWindow {
  ...
  const startWindowLoad = async () => {
    if (!app.isPackaged) {
      await window.webContents.session
        .clearStorageData({
          origin: `${DESKTOP_SCHEME}://app`,
          storages: ["localstorage"],
        })
        .catch(() => undefined);
    }

    await window.loadURL(resolveDesktopRootUrl());
...
async function bootstrap(): Promise<void> {
  ...
  writeDesktopLogHeader(`resolved desktop backend port=${backendPort}`);
  backendAuthToken = Crypto.randomBytes(24).toString("hex");
  const httpBaseUrl = `http://127.0.0.1:${backendPort}`;
  ...
  const backendReadinessController = new AbortController();
  app.once("before-quit", () => {
    backendReadinessController.abort();
  });

  try {
    await waitForHttpReady(httpBaseUrl, {
      signal: backendReadinessController.signal,
    });
    writeDesktopLogHeader(`bootstrap backend ready httpBaseUrl=${httpBaseUrl}`);
  } catch (error) {
    if (isBackendReadinessAborted(error)) {
      writeDesktopLogHeader("bootstrap backend readiness wait aborted");
      return;
    }
    writeDesktopLogHeader(
      `bootstrap backend readiness wait failed httpBaseUrl=${httpBaseUrl} error=${sanitizeLogValue(formatErrorMessage(error))}`,
    );
  }
