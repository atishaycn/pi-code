...
import { mergeStreamingMessageUpdate } from "@t3tools/shared/streamingMessage";
...
function buildSidebarThreadSummary(thread: Thread): SidebarThreadSummary {
  ...
  const latestTurnAssistantMessages = thread.messages.filter(
    (message) => message.role === "assistant" && message.turnId === (latestTurnId ?? null),
  );
  const hasStreamingAssistantMessage = latestTurnAssistantMessages.some(
    (message) => message.streaming,
  );
  const hasAssistantReplyForActiveTurn = hasAssistantReplyForLatestTurn(
    thread.messages,
    latestTurn,
  );
  ...
  return {
    ...
    hasActionableProposedPlan: hasActionableProposedPlan(
      findLatestProposedPlan(thread.proposedPlans, latestTurnId ?? null),
    ),
    isRunningTurn: deriveIsRunningTurn({
      activeLatestTurn: latestTurn,
      latestTurnSettled: isLatestTurnSettled(latestTurn, thread.session),
      sessionOrchestrationStatus: thread.session?.orchestrationStatus,
...
function sidebarThreadSummariesEqual(
  ...
  return (
    ...
    left.isRunningTurn === right.isRunningTurn
  );
}
  ...
  switch (event.type) {
    ...
    case "thread.message-sent": {
      return updateThreadState(state, event.payload.threadId, (thread) => {
        ...
        const existingMessage = thread.messages.find((entry) => entry.id === incomingMessage.id);
        const mergedMessageState = mergeStreamingMessageUpdate(
          existingMessage
            ? {
                text: existingMessage.text,
                streaming: existingMessage.streaming,
                createdAt: existingMessage.createdAt,
                updatedAt: existingMessage.completedAt ?? existingMessage.createdAt,
            ...
            : undefined,
          {
            ...
            updatedAt: event.payload.updatedAt,
            turnId: incomingMessage.turnId ?? null,
        ...
        const mergedMessage: ChatMessage = {
          ...
          text: mergedMessageState.text,
          turnId: mergedMessageState.turnId,
          createdAt: mergedMessageState.createdAt,
          streaming: mergedMessageState.streaming,
          ...(mergedMessageState.streaming ? {} : { completedAt: mergedMessageState.updatedAt }),
          ...(mergedMessageState.attachments !== undefined
            ? { attachments: [...mergedMessageState.attachments] }
            : {}),
        };
        const messages = existingMessage
          ? thread.messages.map((entry) =>
              entry.id !== mergedMessage.id ? entry : { ...entry, ...mergedMessage },
            )
          : [...thread.messages, mergedMessage];
    ...
    case "thread.session-set": {
      return updateThreadState(state, event.payload.threadId, (thread) => {
        ...
        const isSameActiveTurn =
          thread.latestTurn?.turnId === event.payload.session.activeTurnId &&
          event.payload.session.activeTurnId !== null;
        const latestTurnAlreadySettledForSameTurn =
          isSameActiveTurn &&
          (thread.latestTurn?.completedAt !== null ||
            thread.latestTurn?.state === "completed" ||
            thread.latestTurn?.state === "interrupted" ||
            thread.latestTurn?.state === "error" ||
            hasAssistantReplyForLatestTurn(thread.messages, thread.latestTurn));
        ...
        return {
          ...
          latestTurn:
            ...
            event.payload.session.activeTurnId !== null &&
            !latestTurnAlreadySettledForSameTurn
              ? buildLatestTurn({
                  previous: thread.latestTurn,
