...
describe("store read model sync", () => {
  ...
  it("does not keep sidebar threads stuck in working while waiting for a missing final reply", () => {
    const initialState = makeState(makeThread());
    const turnId = TurnId.makeUnsafe("turn-sidebar-timeout");
    const readModel = makeReadModel(
      makeReadModelThread({
        latestTurn: {
          turnId,
          ...
          assistantMessageId: MessageId.makeUnsafe("assistant-missing"),
        },
        activities: [
          {
            id: EventId.makeUnsafe("activity-sidebar-timeout"),
            tone: "tool",
            kind: "tool.completed",
            summary: "Edited file",
            payload: {},
    ...
    expect(next.sidebarThreadsById[ThreadId.makeUnsafe("thread-1")]?.isRunningTurn).toBe(false);
  });
...
describe("incremental orchestration updates", () => {
  ...
  it("does not reopen a completed assistant message when a late streaming delta arrives", () => {
  ...
  it("does not let a late running session-set reopen a completed latest turn", () => {
