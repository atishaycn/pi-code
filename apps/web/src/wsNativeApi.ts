...
export function createWsNativeApi(): NativeApi {
  ...
  const api: NativeApi = {
    ...
    shell: {
      ...
      openTerminalWindow: async (input) => {
        if (!window.desktopBridge) {
          throw new Error("Opening a new terminal window requires desktop app.");
        }

        const opened = await window.desktopBridge.openTerminalWindow(input);
        if (!opened) {
          throw new Error("Unable to open terminal window.");
        }
      },
    },
    git: {
    ...
    server: {
      ...
      getComposerSlashCommands: rpcClient.server.getComposerSlashCommands,
      getRoadmapStatus: rpcClient.server.getRoadmapStatus,
      getPiDoctorReport: rpcClient.server.getPiDoctorReport,
      getPiWorkspace: rpcClient.server.getPiWorkspace,
      readPiResource: rpcClient.server.readPiResource,
      writePiResource: rpcClient.server.writePiResource,
    ...
    orchestration: {
      ...
      sendStreamingMessage: (input) =>
        rpcClient.orchestration.sendStreamingMessage(input).then(() => undefined),
