...
describe("derivePendingUserInputs", () => {
  ...
  it("preserves editor metadata for inline editing requests", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "user-input-open-editor",
        createdAt: "2026-02-23T00:00:01.000Z",
        kind: "user-input.requested",
        summary: "User input requested",
        tone: "info",
        payload: {
          requestId: "req-user-input-editor-1",
          responseKind: "editor",
          title: "Edit generated file",
          prefill: "export const answer = 41;\n",
          questions: [
            {
              id: "editor_text",
              header: "Edit generated file",
              question: "Edit generated file",
              options: [
                {
                  label: "Edit response",
...
describe("deriveWorkLogEntries", () => {
  ...
  it("includes work log entries from earlier turns when requested for chat timeline rendering", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "turn-1-complete",
        createdAt: "2026-02-23T00:00:01.000Z",
  ...
  it("keeps reasoning deltas in the work log and collapses adjacent chunks", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "thinking-1",
        createdAt: "2026-02-23T00:00:01.000Z",
    ...
    expect(entries).toEqual([
      expect.objectContaining({
        ...
        tone: "thinking",
        streamKind: "reasoning_text",
        detail: "Let me think this through.",
      }),
    ]);
  });

  it("deduplicates overlapping reasoning snapshots when providers resend cumulative text", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "thinking-overlap-1",
        createdAt: "2026-02-23T00:00:01.000Z",
        kind: "thinking.delta",
        summary: "Thinking",
        tone: "thinking",
        payload: {
          streamKind: "reasoning_text",
          itemId: "item-reasoning-overlap-1",
          detail: "Let me think",
        },
      }),
      makeActivity({
        id: "thinking-overlap-2",
        createdAt: "2026-02-23T00:00:02.000Z",
        kind: "thinking.delta",
        summary: "Thinking",
        tone: "thinking",
  ...
  it("replaces rewritten cumulative reasoning snapshots instead of appending duplicated text", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "thinking-rewrite-1",
        createdAt: "2026-02-23T00:00:01.000Z",
        kind: "thinking.delta",
        summary: "Thinking",
        tone: "thinking",
        payload: {
          streamKind: "reasoning_text",
          itemId: "item-reasoning-rewrite-1",
          detail:
            "**Investigating a potential bug**\n\nI think there's a likely bug because the matcher seems strict.",
        },
      }),
      makeActivity({
        id: "thinking-rewrite-2",
        createdAt: "2026-02-23T00:00:02.000Z",
        kind: "thinking.delta",
        summary: "Thinking",
        tone: "thinking",
        payload: {
          ...
          itemId: "item-reasoning-rewrite-1",
          detail:
            "**Investigating a potential bug**\n\nI think there's a likely bug because the matches function seems too strict. I need to inspect derivesRunningTurn to clarify this.",
        },
      }),
    ];
  ...
  it("replaces same-length rewritten reasoning snapshots with the same heading", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "thinking-same-length-1",
        createdAt: "2026-02-23T00:00:01.000Z",
        kind: "thinking.delta",
        summary: "Thinking",
        tone: "thinking",
        payload: {
          streamKind: "reasoning_text",
          itemId: "item-reasoning-same-length-1",
          detail:
            "**Investigating button issues**\n\nI need to figure out why the buttons't aren working. It could a be pointer or issue unsupported APIs. I search should for native API methods that might be absent in the web context, which could be causing the buttons to be disabled. ensure Inspecting theNativeApi or the contract might help me understand where the problem lies. This is a puzzle I need to solve step by step!",
        },
      }),
      makeActivity({
        id: "thinking-same-length-2",
        createdAt: "2026-02-23T00:00:02.000Z",
        kind: "thinking.delta",
        summary: "Thinking",
        tone: "thinking",
        payload: {
          ...
          itemId: "item-reasoning-same-length-1",
          detail:
            "**Investigating button issues**\n\nI need to figure out why the buttons aren't working. It could be a pointer issue or unsupported APIs. I should search for native API methods that might be absent in the web context, which could be causing the buttons to be disabled. Inspecting the ensureNativeApi or the contract might help me understand where the problem lies. This is a puzzle I need to solve step by step!",
        },
      }),
    ];
  ...
  it("keeps command output deltas in the work log and collapses adjacent chunks", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "output-1",
        createdAt: "2026-02-23T00:00:01.000Z",
  ...
  it("parses structured file-change tool results into a preview, diff, and changed files", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        ...
        payload: {
          ...
          detail: JSON.stringify({
            content: [
              {
                type: "text",
                text: "Successfully replaced 1 block(s) in apps/web/src/components/ChatView.tsx.",
              },
            ],
            details: {
  ...
  it("unwraps structured read results so the UI shows file contents instead of a raw JSON dump", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "read-complete",
        kind: "tool.completed",
    ...
    const [entry] = deriveWorkLogEntries(activities, undefined);
    expect(entry).toMatchObject({
      detail: 'import { foo } from "./foo";\nexport const value = 1;',
      itemType: "mcp_tool_call",
    });
    expect(entry?.preview).toBeUndefined();
    expect(entry?.detail).not.toContain('"content"');
  });

  it("marks structured tool failures as errors", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "edit-failure",
        kind: "tool.completed",
        ...
        payload: {
          ...
          detail: JSON.stringify({
            content: [
              {
                type: "text",
                text: "Could not find the exact text in apps/web/src/components/ChatView.tsx.",
              },
            ],
          }),
        },
    ...
    expect(entry).toMatchObject({
      tone: "error",
      preview: "Could not find the exact text in apps/web/src/components/ChatView.tsx.",
      changedFiles: ["apps/web/src/components/ChatView.tsx"],
    });
  });
...
describe("hasAssistantReplyForLatestTurn", () => {
  it("matches assistant replies by turn id when available", () => {
    expect(
      hasAssistantReplyForLatestTurn(
        [
          {
            ...
            completedAt: "2026-03-29T00:00:06.000Z",
          },
        ],
        {
          turnId: TurnId.makeUnsafe("turn-1"),
  ...
  it("falls back to assistantMessageId when the message is not linked by turn id", () => {
    expect(
      hasAssistantReplyForLatestTurn(
        [
          {
  ...
  it("falls back to turn timing when the assistant reply is visible but missing both links", () => {
    expect(
      hasAssistantReplyForLatestTurn(
        [
          {
            ...
            role: "assistant",
            turnId: null,
            createdAt: "2026-03-29T00:00:08.000Z",
  ...
  it("does not match older assistant replies from previous turns", () => {
    expect(
      hasAssistantReplyForLatestTurn(
        [
          {
            ...
            completedAt: "2026-03-29T00:00:00.500Z",
          },
        ],
        {
          turnId: TurnId.makeUnsafe("turn-1"),
  ...
  it("falls back to requestedAt when startedAt is missing", () => {
    expect(
      hasAssistantReplyForLatestTurn(
        [
          {
            id: MessageId.makeUnsafe("assistant-requested-at"),
            role: "assistant",
            turnId: null,
            createdAt: "2026-03-29T00:00:08.000Z",
            completedAt: "2026-03-29T00:00:09.000Z",
...
describe("isLatestTurnSettled", () => {
  ...
  it("returns true when a running session reports null for the active turn id", () => {
    expect(
      isLatestTurnSettled(latestTurn, {
        orchestrationStatus: "running",
        activeTurnId: null,
...
describe("deriveIsRunningTurn", () => {
  it("does not keep a turn running when the session reports a null active turn id", () => {
    expect(
      deriveIsRunningTurn({
        activeLatestTurn: {
          turnId: TurnId.makeUnsafe("turn-1"),
