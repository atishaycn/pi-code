...
interface ComposerSlashCommandsQueryInput {
...
export const providerQueryKeys = {
  ...
  composerSlashCommands: (input: ComposerSlashCommandsQueryInput) =>
    ["providers", "composerSlashCommands", input.provider, input.cwd ?? null] as const,
};
...
export function composerSlashCommandsQueryOptions(input: ComposerSlashCommandsQueryInput) {
  return queryOptions({
    queryKey: providerQueryKeys.composerSlashCommands(input),
    queryFn: async () => {
      const api = ensureNativeApi();
      if (!input.cwd) {
        throw new Error("Slash commands are unavailable without a workspace.");
      }
      return api.server.getComposerSlashCommands({
        provider: input.provider,
        cwd: input.cwd,
      });
    },
    enabled: (input.enabled ?? true) && !!input.cwd,
    staleTime: 30_000,
    retry: false,
  });
}
