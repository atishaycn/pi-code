...
import { OrchestrationLatestTurn, ProjectId, ThreadId, TurnId } from "@t3tools/contracts";
import {
...
function makeLatestTurn(overrides?: {
  ...
  return {
    turnId: (overrides?.turnId ?? "turn-1") as never,
    state: "completed",
    assistantMessageId: (overrides?.assistantMessageId ?? null) as never,
...
describe("isThreadActivelyRunning", () => {
  it("trusts the derived running-turn state instead of stale session flags", () => {
    expect(
      isThreadActivelyRunning({
        isRunningTurn: false,
      }),
...
describe("resolveThreadStatusPill", () => {
  ...
  it("does not show working for stale running session state while chat-derived state is already settled", () => {
    expect(
      resolveThreadStatusPill({
        thread: {
          ...baseThread,
  ...
  it("does not show working for stale running session state after the assistant reply completed", () => {
    expect(
      resolveThreadStatusPill({
        thread: {
          ...baseThread,
      ...
      resolveThreadStatusPill({
        thread: {
          ...
          session: {
            ...
            status: "running",
            orchestrationStatus: "running",
            activeTurnId: TurnId.makeUnsafe("turn-tool-only"),
          },
        },
      }),
    ).toMatchObject({ label: "Completed", pulse: false });
...
describe("isWorkingThreadStatus", () => {
  it("returns true only for active working states", () => {
    expect(
      isWorkingThreadStatus({
        label: "Working",
        colorClass: "text-sky-600",
    ...
    expect(isWorkingThreadStatus(null)).toBe(false);
  });
});

describe("resolveSidebarThreadStatus", () => {
  it("keeps completed status visible for the active thread", () => {
    expect(
      resolveSidebarThreadStatus(
        {
          label: "Completed",
  ...
  it("keeps completed status visible for inactive threads", () => {
    expect(
      resolveSidebarThreadStatus(
        {
          label: "Completed",
...
describe("sortThreadsForSidebar", () => {
  ...
  it("keeps pinned threads at the top within a project", () => {
    ...
    expect(sorted.map((thread) => thread.id)).toEqual([thread3, thread2, thread1]);
  });
});
...
describe("getFallbackThreadIdAfterDelete", () => {
  ...
  it("prefers a pinned remaining thread as the fallback", () => {
    const pinnedThreadId = ThreadId.makeUnsafe("thread-pinned");
    const fallbackThreadId = getFallbackThreadIdAfterDelete({
      threads: [
        makeThread({
          id: ThreadId.makeUnsafe("thread-active"),
      ...
      deletedThreadId: ThreadId.makeUnsafe("thread-active"),
      pinnedThreadIds: new Set([pinnedThreadId]),
      sortOrder: "created_at",
    });

    expect(fallbackThreadId).toBe(pinnedThreadId);
  });
});
