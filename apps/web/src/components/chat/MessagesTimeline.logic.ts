...
function estimateWorkRowHeight(
  ...
  const hasOverflow = canCollapse && row.groupedEntries.length > MAX_VISIBLE_WORK_LOG_ENTRIES;
  ...
  return (
    28 +
    (showHeader ? 26 : 0) +
    row.groupedEntries.slice(0, visibleEntries).reduce((height, entry) => {
      const detailText = typeof entry.detail === "string" ? entry.detail : null;
      const detailBlockVisible =
        detailText !== null &&
        detailText.length > 0 &&
        !entry.command &&
        (entry.streamKind !== undefined || entry.tone === "thinking" || detailText.includes("\n"));
      const detailLines =
        detailBlockVisible && detailText !== null
          ? Math.max(1, Math.ceil(detailText.length / 96))
          : 0;
      return height + 32 + (detailBlockVisible ? Math.min(180, detailLines * 18 + 12) : 0);
    }, 0)
  );
}
