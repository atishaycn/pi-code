...
import { PiWorkspaceSection } from "./PiWorkspaceSection";
import {
...
const PROVIDER_SETTINGS: readonly InstallProviderSettings[] = [
  {
    ...
    binaryDescription:
      "Path to the pi launcher executable or script (for example pi, ./pi.sh, or /opt/pi/bin/pi)",
    homePathKey: "codexHomePath",
    homePlaceholder: "PI_CODING_AGENT_DIR",
    homeDescription: "Optional custom pi config directory.",
  },
  {
...
function getProviderSummary(provider: ServerProvider | undefined) {
  ...
  if (!provider.enabled) {
    return {
      headline: "Disabled",
      detail:
        provider.message ?? "This provider is installed but disabled for new sessions in Pi Code.",
    };
  }
  ...
  const changedSettingLabels = useMemo(
    () => [
      ...
      ...(settings.diffWordWrap !== DEFAULT_UNIFIED_SETTINGS.diffWordWrap
        ? ["Diff line wrapping"]
        : []),
      ...(settings.skipWorkWhileScrolling !== DEFAULT_UNIFIED_SETTINGS.skipWorkWhileScrolling
        ? ["Skip work while scrolling"]
        : []),
    ...
    [
      ...
      settings.skipWorkWhileScrolling,
...
export function GeneralSettingsPanel() {
  ...
  const [openProviderDetails, setOpenProviderDetails] = useState<Record<ProviderKind, boolean>>({
    codex: Boolean(
      ...
      settings.providers.codex.enableAutoreason !==
        DEFAULT_UNIFIED_SETTINGS.providers.codex.enableAutoreason ||
      settings.providers.codex.fullAutonomy !==
        DEFAULT_UNIFIED_SETTINGS.providers.codex.fullAutonomy ||
  ...
  const piDoctorReportQuery = useQuery({
    queryKey: ["server", "pi-doctor-report"],
    queryFn: () => ensureNativeApi().server.getPiDoctorReport(),
    staleTime: 15_000,
  });
  ...
  const diagnosticsDescription = (() => {
    ...
    const doctorSummary = piDoctorReportQuery.data
      ? ` Pi doctor: ${piDoctorReportQuery.data.summary.error} errors, ${piDoctorReportQuery.data.summary.warning} warnings.`
      : piDoctorReportQuery.isLoading
        ? " Pi doctor: checking coding runtime health..."
        : "";
    const base =
      exports.length > 0 ? `${mode}. OTLP exporting ${exports.join(" and ")}.` : `${mode}.`;
    return `${base}${doctorSummary}`;
  })();
  ...
  const topDoctorDiagnostic = piDoctorReportQuery.data?.diagnostics[0] ?? null;
  ...
  return (
    <SettingsPageContainer>
      <SettingsSection title="General">
        <SettingsRow
          title="Theme"
          description="Choose how Pi Code looks across the app."
          resetAction={
            theme !== "system" ? (
        ...
        <SettingsRow
          title="Skip work while scrolling"
          description="When scrolling the chat, jump past thinking and tool-call rows to the next conversation item."
          resetAction={
            settings.skipWorkWhileScrolling !== DEFAULT_UNIFIED_SETTINGS.skipWorkWhileScrolling ? (
              <SettingResetButton
                label="skip work while scrolling"
                onClick={() =>
                  updateSettings({
                    skipWorkWhileScrolling: DEFAULT_UNIFIED_SETTINGS.skipWorkWhileScrolling,
                  })
                }
              />
            ) : null
          ...
          control={
            <Switch
              checked={settings.skipWorkWhileScrolling}
              onCheckedChange={(checked) =>
                updateSettings({ skipWorkWhileScrolling: Boolean(checked) })
              }
              aria-label="Skip thinking and tool rows while scrolling"
            />
          }
        />
        ...
        <SettingsRow
          ...
          description="Configure the model used for generated commit messages, PR titles, branch names, and chat titles."
          resetAction={
            isGitWritingModelDirty ? (
              <SettingResetButton
                          ...
                          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div className="min-w-0 flex-1 space-y-1">
                              ...
                              <div className="text-xs text-muted-foreground">
                                Off by default. Only works with the <code>pi-test.sh</code> wrapper.
                                When off, Pi Code launches that wrapper without autoreason support.
                              </div>
                            </div>
                            <Switch
                              checked={settings.providers.codex.enableAutoreason}
                              onCheckedChange={(checked) =>
                                updateSettings({
                                  providers: {
                                    ...settings.providers,
                                    codex: {
                                      ...settings.providers.codex,
                                      enableAutoreason: Boolean(checked),
                                    },
                                  },
                                })
                              }
                              aria-label="Enable autoreason for pi"
                            />
                          </div>
                        </div>
                        ...
                        <div className="border-t border-border/60 px-4 py-3 sm:px-5">
                          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div className="min-w-0 flex-1 space-y-1">
                              ...
                              <div className="text-xs text-muted-foreground">
                                Pass <code>--full-autonomy</code> to embedded pi sessions. Pushes
                                the model harder toward end-to-end execution. Applies to newly
                                started sessions.
                              </div>
                            </div>
                            <Switch
                              checked={settings.providers.codex.fullAutonomy}
                              onCheckedChange={(checked) =>
                                updateSettings({
                                  providers: {
                                    ...settings.providers,
                                    codex: {
                                      ...settings.providers.codex,
                                      fullAutonomy: Boolean(checked),
                                    },
                                  },
                                })
                              }
                              aria-label="Enable full autonomy for pi"
                            />
                          </div>
                        </div>
                      </>
      ...
      <SettingsSection title="Pi customization">
        <PiWorkspaceSection />
      </SettingsSection>
        ...
        <SettingsRow
          ...
          status={
            <>
              ...
              {piDoctorReportQuery.data ? (
                <span className="mt-1 block text-[11px] text-muted-foreground">
                  Doctor report: {piDoctorReportQuery.data.summary.total} checks,{" "}
                  {piDoctorReportQuery.data.summary.error} errors,{" "}
                  {piDoctorReportQuery.data.summary.warning} warnings.
                </span>
              ) : null}
              {topDoctorDiagnostic ? (
                <span className="mt-1 block text-[11px] text-muted-foreground">
                  Top issue: {topDoctorDiagnostic.summary}
                </span>
              ) : null}
              {piDoctorReportQuery.error instanceof Error ? (
                <span className="mt-1 block text-destructive">
                  {piDoctorReportQuery.error.message}
                </span>
              ) : null}
