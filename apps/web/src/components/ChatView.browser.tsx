...
function createThread(options: {
  ...
  return {
    ...
    branch: "main",
    worktreePath: null,
    latestTurn: options.latestTurn ?? null,
    createdAt: options.createdAt ?? NOW_ISO,
    ...
    session: {
      ...
      providerName: "codex",
      runtimeMode: "full-access",
      activeTurnId: options.latestTurn?.turnId ?? null,
      lastError: null,
      updatedAt: options.updatedAt ?? NOW_ISO,
    },
  };
...
function addThreadToSnapshot(
  ...
  return {
    ...
    threads: [...snapshot.threads, createThread({ threadId, title: "New thread" })],
  };
}
...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it("keeps completed chats settled when a late assistant delta arrives", async () => {
    const completedTurnId = "turn-late-streaming-tail" as TurnId;
    const assistantMessageId = "assistant-late-streaming-tail" as MessageId;
    const snapshot = createSnapshotForTargetUser({
      targetMessageId: "msg-user-late-streaming-tail" as MessageId,
      targetText: "late streaming tail target",
    });
    const targetThreadIndex = snapshot.threads.findIndex((thread) => thread.id === THREAD_ID);
    const threads = [...snapshot.threads];
    if (targetThreadIndex >= 0) {
      const thread = snapshot.threads[targetThreadIndex]!;
      threads[targetThreadIndex] = {
        ...thread,
        messages: [
          createUserMessage({
            id: "msg-user-late-streaming-tail" as MessageId,
            text: "late streaming tail target",
            offsetSeconds: 1_000,
          }),
          {
            ...createAssistantMessage({
        ...
        latestTurn: {
          turnId: completedTurnId,
          state: "completed",
          requestedAt: isoAt(1_009),
          startedAt: isoAt(1_010),
          completedAt: isoAt(1_011),
          assistantMessageId,
        },
        session:
          thread.session === null
    ...
    try {
      ...
      sendOrchestrationDomainEvent({
        sequence: 2,
        eventId: EventId.makeUnsafe("evt-late-streaming-tail"),
        aggregateKind: "thread",
        aggregateId: THREAD_ID,
        occurredAt: isoAt(1_012),
        commandId: null,
        causationEventId: null,
        correlationId: null,
        metadata: {},
        ...
        payload: {
          ...
          turnId: completedTurnId,
          streaming: true,
          createdAt: isoAt(1_012),
          updatedAt: isoAt(1_012),
        },
      });

      await vi.waitFor(
        () => {
          expect(document.body.textContent).toContain("done tail");
          expect(document.querySelector('[data-testid="processing-status-panel"]')).toBeNull();
  ...
  it("does not show a stale Working badge in the sidebar after the assistant reply is already visible", async () => {
    const runningTurnId = "turn-sidebar-stale-working" as TurnId;
    const assistantMessageId = "assistant-sidebar-stale-working" as MessageId;
    const baseSnapshot = createSnapshotForTargetUser({
      targetMessageId: "msg-user-sidebar-stale-working" as MessageId,
      targetText: "sidebar stale working target",
    });
    ...
    if (targetThreadIndex >= 0) {
      ...
      threads[targetThreadIndex] = {
        ...
        messages: [
          createUserMessage({
            id: "msg-user-sidebar-stale-working" as MessageId,
            text: "sidebar stale working target",
            offsetSeconds: 2_000,
          }),
          {
            ...createAssistantMessage({
  ...
  it("shows the rename title action in the sidebar and opens inline editing", async () => {
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-rename-hover-test" as MessageId,
        targetText: "rename hover target",
      }),
    });
    ...
    try {
      ...
      const renameButton = page.getByTestId(`thread-rename-${THREAD_ID}`);
      ...
      await vi.waitFor(
        () => {
          ...
          expect(renameInput?.value).toBe("Browser test thread");
        },
        { timeout: 4_000, interval: 16 },
      );
    } finally {
