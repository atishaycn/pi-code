...
> & {
  ...
  isManuallyCompleted?: boolean | undefined;
};

export function isThreadActivelyRunning(thread: Pick<ThreadStatusInput, "isRunningTurn">): boolean {
  return thread.isRunningTurn;
}
...
export function isWorkingThreadStatus(status: ThreadStatusPill | null): boolean {
...
>(
  ...
  pinnedThreadIds?: ReadonlySet<T["id"]>,
): T[] {
  return threads.toSorted((left, right) => {
    const leftPinned = pinnedThreadIds?.has(left.id) ?? false;
    const rightPinned = pinnedThreadIds?.has(right.id) ?? false;
...
>(input: {
  ...
  const { deletedThreadId, deletedThreadIds, pinnedThreadIds, sortOrder, threads } = input;
