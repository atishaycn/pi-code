...
const rpcClientMock = {
  ...
  server: {
    ...
    getComposerSlashCommands: vi.fn(),
    getRoadmapStatus: vi.fn(),
    getPiDoctorReport: vi.fn(),
    getPiWorkspace: vi.fn(),
...
describe("wsNativeApi", () => {
  ...
  it("forwards streaming steer messages to the orchestration RPC", async () => {
    rpcClientMock.orchestration.sendStreamingMessage.mockResolvedValue({});
    const { createWsNativeApi } = await import("./wsNativeApi");

    const api = createWsNativeApi();
    await api.orchestration.sendStreamingMessage({
      threadId: ThreadId.makeUnsafe("thread-1"),
      input: "redirect current turn",
      attachments: [],
      streamingBehavior: "steer",
    });

    expect(rpcClientMock.orchestration.sendStreamingMessage).toHaveBeenCalledWith({
      threadId: "thread-1",
      input: "redirect current turn",
      attachments: [],
      streamingBehavior: "steer",
  ...
  it("forwards pi doctor report fetches directly to the RPC client", async () => {
    const doctorReport = {
      version: 1,
      ok: false,
    ...
    rpcClientMock.server.getPiDoctorReport.mockResolvedValue(doctorReport);
    const { createWsNativeApi } = await import("./wsNativeApi");

    const api = createWsNativeApi();

    await expect(api.server.getPiDoctorReport()).resolves.toEqual(doctorReport);
    expect(rpcClientMock.server.getPiDoctorReport).toHaveBeenCalledWith();
  });
  ...
  it("forwards composer slash command lookups directly to the RPC client", async () => {
    rpcClientMock.server.getComposerSlashCommands.mockResolvedValue({
      commands: [{ name: "commands", source: "extension", description: "List commands" }],
    });
    const { createWsNativeApi } = await import("./wsNativeApi");
    ...
    await expect(
      api.server.getComposerSlashCommands({ provider: "codex", cwd: "/repo" }),
    ).resolves.toEqual({
      commands: [{ name: "commands", source: "extension", description: "List commands" }],
    });
    expect(rpcClientMock.server.getComposerSlashCommands).toHaveBeenCalledWith({
      provider: "codex",
      cwd: "/repo",
    });
  });

  it("opens desktop terminal windows through the desktop bridge", async () => {
    const openTerminalWindow = vi.fn(async () => true);
    getWindowForTest().desktopBridge = makeDesktopBridge({ openTerminalWindow });

    const { createWsNativeApi } = await import("./wsNativeApi");
    const api = createWsNativeApi();

    await expect(
      api.shell.openTerminalWindow({
        cwd: "/tmp/workspace",
        command: "echo hello",
        title: "Implement current step",
      }),
