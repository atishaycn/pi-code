...
function coalesceOrchestrationUiEvents(
  ...
  for (const event of events) {
    ...
    if (
      ...
      const mergedMessage = mergeStreamingMessageUpdate(
        {
          text: previous.payload.text,
          streaming: previous.payload.streaming,
          createdAt: previous.payload.createdAt,
          updatedAt: previous.payload.updatedAt,
          turnId: previous.payload.turnId,
          ...(previous.payload.attachments !== undefined
            ? { attachments: previous.payload.attachments }
            : {}),
        },
        {
...
function EventRouter() {
  ...
  useEffect(() => {
    ...
    const applyEventBatch = (events: ReadonlyArray<OrchestrationEvent>) => {
      ...
      useRoadmapLiveState.getState().recordEvents(nextEvents);
    ...
    if (!useStore.getState().bootstrapComplete) {
      void bootstrapFromSnapshot().catch(() => undefined);
    }
